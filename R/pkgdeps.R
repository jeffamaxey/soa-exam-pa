...
get_pkgdeps <- function(root_dir = getwd(),
                        ignores = NULL,
                        verbose = TRUE) {
  if (!dir.exists(root_dir))
    stop(glue::glue("Invalid `root_dir` argument. `{root_dir}` does not exist."))
  init_pkg_names <- get_dependent_packages(root_dir)
  if (length(init_pkg_names) == 0) {
    cli::cli_alert_warning("warning: no packages found in specified directory")
    return(invisible(NULL))
  }
  hold <- suppressWarnings(lapply(
    init_pkg_names,
    purrr::safely(get_package_details, quiet = TRUE)
  ))
...
#'
#' @description
#' This function creates a specifically structured `YAML` file to house an R
#' projects package dependencies.
#'
#' @param deps list returned from [get_pkgdeps()]; leave null to run [get_pkgdeps()].
#' @param root_dir the package directory path
...
create_pkgdeps_file <- function(deps = NULL,
                                root_dir = getwd(),
                                ...) {
  if (is.null(deps)) {
    #msg_info("Retrieving package dependencies from {msg_path(root_dir)}...")
    deps <- get_pkgdeps(root_dir = root_dir, ...)
    #msg_info("Found {length(deps)} package dependencies.")
  }
  yaml::write_yaml(deps, file.path(root_dir, "deps.yml"))
  msg_done("Successfully created `deps.yml` for app at {msg_path(root_dir)}.")
}
...
get_package_deps <- function(path = getwd(),
  ...
  if (write_yaml) {
    ...
    cli::cat_bullet("Created file `deps.yaml`.",
                    bullet = "tick",
                    bullet_col = "green")
  }
  ...
  if (write_r) {
    ...
    cli::cat_bullet("Created file `deps.R`.",
                    bullet = "tick",
                    bullet_col = "green")
  }

  out_df <- df %>% dplyr::select(package = Package,
                                 src = Repository,
                                 version = Version,
                                 install_cmd)
